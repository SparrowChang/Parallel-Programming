...
//void init_line(void); delete and do parallel
void update (void);
...
/* init_line() and Calculate initial values based on sine curve */
   fac = 2.0 * PI;
   k = 1 + blockIdx.x * BLOCK_SIZE + threadIdx.x; /* key */ 
   tmp = tpoints - 1;
   x = (k - 1) / tmp;
   value = sin (fac * x);
   old_val = value;

/* do math() and Calculate new values using wave equation*/ 
   dtime = 0.3;
   c = 1.0;
   dx = 1.0;
   tau = (c * dtime / dx);
...
/* update() and Update all values along line a specified number of times */ 
    if(k <= tpoints) {
      for (i = 1; i<= nsteps; i++) {
        if ((k == 1) || (k  == tpoints))
          new_val = 0.0;
